...
public class RequestDTO {
	private int requestCount; // 전체 글 개수
	private int currentPage; // 현재 보고있는 페이지
	private int totalPages; // 전체 페이지 개수
	private int startPage; // 시작 페이지 번호
	private int endPage; // 끝 페이지 번호
	private static final int NAV_NUM = 5; // 페이지는 10개씩 링크 표시(하단에 깔릴 버튼은 10개씩)
	private static final int REQUEST_NUM = 10; // 글도 10개씩 표시(DAO의 limit 구문도 함께 수정해야함)
	
	// 생성자 생성시 모든 정보를 자동으로 구하도록 처리
	public RequestDTO(int requestCount, int currentPage) {
	...
	public int getRequestCount() {
		return requestCount; // 총 글 개수
	}
	
	// 밑에 버튼을 깔지 말지를 결정하기 위한 bool 자료형
	public boolean hasNoBoard() {
		return requestCount == 0; //표시할 게시물이 없으면 버튼도 깔 필요 없음
	}
	
	public boolean hasBoard() {
		return requestCount > 0; // 표시할 게시물이 있다면 버튼을 깔아야함
	}
	...
	public String toString() {
		return "RequestDTO [requestCount=" + requestCount + ", currentPage=" + currentPage + ", totalPages=" + totalPages
